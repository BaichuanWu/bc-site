...
import type { CanvasNodeData } from "@/components/workflow/canvas/serializer"
...
function toCanvasNodeData(value: unknown): CanvasNodeData {
  ...
  return {
    ...
    agentDisplay: typeof source.agentDisplay === "string" ? source.agentDisplay : undefined,
    agentVersionDisplay:
      typeof source.agentVersionDisplay === "string"
        ? source.agentVersionDisplay
        : undefined,
    agentClassDisplay:
      typeof source.agentClassDisplay === "string"
        ? source.agentClassDisplay
        : undefined,
  }
}
      ...
      <div className="space-y-3">
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            ...
            <div className="line-clamp-2 text-xs text-muted-foreground">
              {safeData.agentDisplay || safeData.label || "Workflow node"}
            </div>
          </div>
        ...
        <div className="space-y-1 rounded-2xl bg-muted/40 px-3 py-2 text-xs">
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <span className="font-medium">{safeData.kind || kind}</span>
          </div>
          <div className="flex items-center justify-between gap-3">
            <span className="text-muted-foreground">Class</span>
            <span className="font-medium">{safeData.agentClassDisplay || "-"}</span>
          </div>
        </div>
      </div>
      <Handle id="out" type="source" position={Position.Right} className={handleClassName} style={handleStyle} />
